...
async function paginate(items, page, limit) {
    const total = items.length;
    const pages = Math.max(1, Math.ceil(total / limit));
    const start = (page - 1) * limit;
    return { items: items.slice(start, start + limit), total, page, pages };
}
function pageOf(req, fallbackLimit = 12) {
...
async function signApiToken(user) {
...
async function apiUser(req, res) {
    if (res.locals.__apiUserChecked)
        return res.locals.__apiUser ?? null;
...
app.post("/api/auth/logout", ah(async (_req, res) => {
...
app.get("/api/products", ah(async (req, res) => {
    ...
    for (const p of items)
        enriched.push(await enrichProduct(p));
    res.json(await paginate(enriched, page, limit));
}));
app.get("/api/products/featured", ah(async (_req, res) => {
    const docs = await products().find({ hidden: { $ne: true }, featured: true }).toArray();
    const items = [];
    for (const p of docs)
        items.push(await enrichProduct(p));
    items.sort((a, b) => (b.averageRating ?? 0) - (a.averageRating ?? 0));
    res.json({ items });
}));
app.get("/api/products/best-sellers", ah(async (_req, res) => {
...
app.get("/api/products/categories", ah(async (_req, res) => {
    const docs = await products()
        .find({ hidden: { $ne: true }, category: { $exists: true, $ne: "" } })
        .toArray();
...
app.get("/api/products/:id/reviews", ah(async (req, res) => {
...
app.post("/api/products/:id/reviews", ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    const oid = toOid(req.params.id);
...
// Wishlist
// ------------------------------------------------------------
app.get("/api/wishlist", ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    const doc = await wishlists().findOne({ userId: user._id.toString() });
    const ids = (doc?.productIds ?? []).map(toOid).filter((x) => x !== null);
...
app.post("/api/wishlist", ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    ...
    await wishlists().updateOne({ userId }, { $addToSet: { productIds: productId } }, { upsert: true });
    res.json({ success: true });
}));
app.delete("/api/wishlist/:productId", ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    await wishlists().updateOne({ userId: user._id.toString() }, { $pull: { productIds: req.params.productId } });
    res.json({ success: true });
}));
// ------------------------------------------------------------
// Orders
// ------------------------------------------------------------
app.post("/api/orders/checkout", ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    const body = (req.body ?? {});
...
app.post("/api/orders/:id/confirm", ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    const oid = toOid(req.params.id);
    ...
    if (stripe && sessionId) {
        ...
        await orders().updateOne({ _id: order._id }, { $set: { paymentStatus: "paid", status: "processing", orderStatus: "processing" } });
        return res.json({
...
app.get("/api/orders/my", ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    const { page, limit } = pageOf(req, 10);
    const mine = (await orders().find({ "buyer._id": user._id.toString() }).toArray()).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
...
app.post("/api/orders/:id/cancel", ah(async (req, res) => {
    ...
    await orders().updateOne({ _id: order._id }, { $set: { status: "cancelled", orderStatus: "cancelled", paymentStatus: "refunded" } });
    res.json({ order: toOrder({ ...order, status: "cancelled", orderStatus: "cancelled", paymentStatus: "refunded" }) });
}));
// ------------------------------------------------------------
// Reviews
// ------------------------------------------------------------
app.get("/api/reviews/latest", ah(async (_req, res) => {
...
// Seller
// ------------------------------------------------------------
app.get("/api/seller/overview", requireRoles("seller", "admin"), ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    ...
    const myOrderDocs = await orders()
        .find({ status: { $ne: "cancelled" }, "items.seller._id": sellerId })
        .toArray();
...
app.get("/api/seller/analytics", requireRoles("seller", "admin"), ah(async (req, res) => {
...
app.get("/api/seller/products", requireRoles("seller", "admin"), ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    const docs = (await products().find({ sellerId: user._id.toString() }).toArray()).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
    const items = [];
    for (const p of docs) {
...
app.post("/api/seller/products", requireRoles("seller", "admin"), ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    const body = (req.body ?? {});
...
app.patch("/api/seller/products/:id", requireRoles("seller", "admin"), ah(async (req, res) => {
    const oid = toOid(req.params.id);
    ...
    if (body.discountPrice != null)
        patch.discountPrice = Number(body.discountPrice);
...
app.delete("/api/seller/products/:id", requireRoles("seller", "admin"), ah(async (req, res) => {
    const oid = toOid(req.params.id);
    if (!oid)
        return res.status(404).json({ message: "Product not found" });
...
app.get("/api/seller/orders", requireRoles("seller", "admin"), ah(async (req, res) => {
    const user = await apiUser(req, res);
    if (!user)
        return res.status(401).json({ message: "Not authenticated" });
    const { page, limit } = pageOf(req, 10);
    const sellerId = user._id.toString();
    const mine = (await orders().find({ "items.seller._id": sellerId }).toArray()).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
...
app.patch("/api/seller/orders/:id/status", requireRoles("seller", "admin"), ah(async (req, res) => {
    const oid = toOid(req.params.id);
    const order = oid ? await orders().findOne({ _id: oid }) : null;
    if (!order)
        return res.status(404).json({ message: "Order not found" });
...
app.post("/api/seller/apply", requireRoles("seller", "admin"), ah(async (_req, res) => {
    res.json({ applied: true });
}));
// ------------------------------------------------------------
// Admin
// ------------------------------------------------------------
app.get("/api/admin/overview", requireRoles("admin"), ah(async (_req, res) => {
...
app.get("/api/admin/users", requireRoles("admin"), ah(async (req, res) => {
    const { page, limit } = pageOf(req, 15);
    const docs = (await users().find({}).toArray()).sort((a, b) => new Date(b.createdAt ?? 0).getTime() - new Date(a.createdAt ?? 0).getTime());
...
app.patch("/api/admin/users/:id/role", requireRoles("admin"), ah(async (req, res) => {
...
app.patch("/api/admin/users/:id/block", requireRoles("admin"), ah(async (req, res) => {
...
app.delete("/api/admin/users/:id", requireRoles("admin"), ah(async (req, res) => {
    const oid = toOid(req.params.id);
    const user = oid ? await users().findOne({ _id: oid }) : null;
    if (!user)
        return res.status(404).json({ message: "User not found" });
...
app.get("/api/admin/products", requireRoles("admin"), ah(async (req, res) => {
    const { page, limit } = pageOf(req, 15);
    const docs = (await products().find({}).toArray()).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
    const totalPages = Math.max(1, Math.ceil(docs.length / limit));
    const start = (page - 1) * limit;
    res.json({
...
app.patch("/api/admin/products/:id/visibility", requireRoles("admin"), ah(async (req, res) => {
    const oid = toOid(req.params.id);
...
app.delete("/api/admin/products/:id", requireRoles("admin"), ah(async (req, res) => {
    const oid = toOid(req.params.id);
    if (!oid)
        return res.status(404).json({ message: "Product not found" });
    await products().deleteOne({ _id: oid });
...
app.get("/api/admin/orders", requireRoles("admin"), ah(async (req, res) => {
    const { page, limit } = pageOf(req, 15);
    const docs = (await orders().find({}).toArray()).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
    const totalPages = Math.max(1, Math.ceil(docs.length / limit));
    const start = (page - 1) * limit;
    res.json({
...
app.patch("/api/admin/orders/:id/status", requireRoles("admin"), ah(async (req, res) => {
    const oid = toOid(req.params.id);
    const order = oid ? await orders().findOne({ _id: oid }) : null;
    if (!order)
        return res.status(404).json({ message: "Order not found" });
